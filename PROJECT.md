...
- **Cart = zero-auth Shopify permalink (operator decision)** — the customer link is `/cart/{variant}:{qty}` which loads their real on-site cart; Sarah has full add/remove/replace/clear control via add_to_cart/set_cart regenerating the link. A draft-order sub-workflow (workflows/shopify-cart.json, OAuth2 cred, proven working) exists but is dormant — revive only if the estate later wants every cart recorded as a Shopify draft.
